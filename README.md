...
A simple Solana onchain arbitrage bot for arbitrage opportunities. This bot calculates the most optimal trade size between various DEX pools on Solana and executes trades when profitable opportunities are found. This repository utilizes the onchain program for executing arbitrage trades.

**This is a demo bot to show how to parse each pool and call the onchain program.**
**This is NOT a fully functional bot. This is only recommended for advanced users to use as a reference.**
...
1. Clone the repository
   ...
   ```
   git clone https://github.com/lerry/solana-arb-bot.git
   cd solana-arb-bot
   ```
...
3. Run the bot
   ```
   cargo run --release --bin solana-arb-bot -- --config config.toml
   ```
